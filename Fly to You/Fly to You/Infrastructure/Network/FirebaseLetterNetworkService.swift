//
//  FirebaseLetterNetworkService.swift
//  Fly to You
//
...
final class FirebaseLetterNetworkService: LetterNetworkService {
    ...
    init(sessionService: UserSessionService, throttlerService: ThrottlerService) {
    ...
    func saveLetter(_ letter: Letter) async throws -> Letter {
        return try await withCheckedThrowingContinuation { continuation in
            throttlerService {
                Task {
                    do {
                        let document = self.db.collection("letters").document(letter.id)
